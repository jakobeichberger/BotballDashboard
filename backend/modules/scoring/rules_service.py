...
async def apply_head_to_head(db: AsyncSession, scheduled_match_id: str, season_id: str) -> dict:
    ...
    score and result (win / loss / replay) onto the schedule. Matches with only
    one team (seeding) are left alone.
    """
    ...
    participants = await db.execute(
        select(MatchParticipant).where(MatchParticipant.scheduled_match_id == scheduled_match_id)
    )
    totals = {m.team_id: m.total_score for m in matches}
    for participant in participants.scalars():
        if participant.team_id not in totals:
            continue
        participant.score = totals[participant.team_id]
        if outcome.replay:
            participant.result = "replay"
        else:
            participant.result = "win" if participant.team_id == outcome.winner else "loss"
    await db.flush()
