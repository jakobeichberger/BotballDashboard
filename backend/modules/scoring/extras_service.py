...
async def de_placement(db: AsyncSession, event_id: str) -> list[dict[str, Any]]:
    ...
    A tie-breaker's value for a team is summed over its head-to-head matches of
    the event's double-elimination phases; the seeding rank is the last resort.
    """
    event = await db.get(Event, event_id)
    if not event:
        raise NotFoundError("Event not found")
    ...
    de_phase_ids = set(
        (
            await db.execute(
                select(EventPhase.id).where(
                    EventPhase.event_id == event_id,
                    EventPhase.phase_type.in_(("double_elimination", "final")),
                )
            )
        ).scalars()
    )
    per_team: dict[str, list[dict[str, float | None]]] = {}
    if de_phase_ids and criteria:
        matches = await db.execute(
            select(Match).where(
                Match.event_id == event_id,
                Match.event_phase_id.in_(de_phase_ids),
                Match.team_id.in_(team_ids),
                Match.is_practice.is_(False),
            )
        )
        for m in matches.scalars():
            per_team.setdefault(m.team_id, []).append(
    ...
    ranking_rows = await db.execute(
        select(Ranking.team_id, Ranking.rank).where(
            Ranking.event_id == event_id, Ranking.team_id.in_(team_ids)
        )
    )
    ...
    for bracket in sorted({r.bracket for r in rows}):
        items = [
            tiebreak.RankedItem(
                id=r.team_id,
                # Lower DE rank is better; rank_with_tiebreakers sorts high→low.
                score=-float(r.de_rank) if r.de_rank else float("-inf"),
                values=tiebreak.sum_values(per_team.get(r.team_id, [])),
                fallback=float(seed_ranks[r.team_id]) if r.team_id in seed_ranks else None,
            )
            for r in rows
            if r.bracket == bracket
        ]
        by_team = {r.team_id: r for r in rows if r.bracket == bracket}
            ...
            out.append(
                {
                    "bracket": bracket,
                    "team_id": item.id,
                    "team_name": names[item.id].name if item.id in names else None,
                    "de_rank": row.de_rank,
                    "placement": item.rank,
