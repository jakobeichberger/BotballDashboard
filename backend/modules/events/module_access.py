...
async def _resolve_paper(db: AsyncSession, path: dict) -> tuple[str | None, str | None]:
    ...
    return (paper.event_id, paper.season_id) if paper else (None, None)
...
def require_season_event_module(module: str) -> Callable[..., Awaitable[None]]:
    """Guard for season-scoped scoring routes (DE, aerial, documentation).

    They read and write the event named by ``?event_id=`` or else the season's
    default event, so that is the event whose switch applies.
    """
