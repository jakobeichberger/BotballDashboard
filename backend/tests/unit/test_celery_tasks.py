...
Each task wraps its work in ``asyncio.run`` with its own session from
``AsyncSessionLocal``. The tests hand the task the test session instead and
run the coroutine on the test's event loop, so the task bodies (and what they
write) are checked without a broker, a worker or a second database.
"""
...
class _SessionContext:
    """Stands in for ``AsyncSessionLocal()``: yields the test session, never closes it."""
...
def run_task(db, monkeypatch):
    """Call a task function and await the coroutine it would have passed to asyncio.run."""

    async def run(module, task, *args):
        captured: list = []
        fake_asyncio = types.SimpleNamespace(run=captured.append, to_thread=asyncio.to_thread)
        monkeypatch.setattr(module, "asyncio", fake_asyncio)
        monkeypatch.setattr(module, "AsyncSessionLocal", lambda: _SessionContext(db))
        task(*args)
        assert len(captured) == 1, "the task must hand exactly one coroutine to asyncio.run"
        await captured[0]
