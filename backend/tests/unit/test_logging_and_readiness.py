...
def production_logging(monkeypatch):
    ...
    monkeypatch.setattr(
        app_logging,
        "get_settings",
        lambda: types.SimpleNamespace(log_level="", is_dev=False, db_echo=False),
    )
...
def test_sql_statements_are_logged_only_with_db_echo(monkeypatch):
    engine_logger = logging.getLogger("sqlalchemy.engine")
    saved = engine_logger.level
    root = logging.getLogger()
    saved_root = (root.handlers[:], root.level)
    try:
        for db_echo, expected in ((True, logging.INFO), (False, logging.WARNING)):
            monkeypatch.setattr(
                app_logging,
                "get_settings",
                lambda db_echo=db_echo: types.SimpleNamespace(
                    log_level="", is_dev=True, db_echo=db_echo
                ),
            )
            app_logging.configure_logging()
            assert engine_logger.level == expected
    finally:
        monkeypatch.undo()
        engine_logger.setLevel(saved)
