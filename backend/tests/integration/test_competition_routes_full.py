...
class TestScoreSheetRoutes:
    ...
    async def test_upload_survives_a_missing_broker_and_logs_it(
        self, client, db, auth_headers, season, monkeypatch
    ):
        ...
        # The task is sent after the commit (get_db's, here the test's), in a
        # worker thread.
        await db.commit()
        await task_queue.drain_pending_tasks()
        ...
        assert event == "task_enqueue_failed"
        assert fields["task"] == "score_sheets.extract_template"
