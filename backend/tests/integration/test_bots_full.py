...
class TestBotCrud:
    ...
    async def test_create_team_bot(self, client, auth_headers, season, team):
        r = await client.post(
            ...
            json={
                ...
                "team_id": team.id,
                "season_id": season.id,
                "description": "Unser Bot",
                "functionality": "Differentialantrieb, Servo-Greifer",
    ...
    async def test_create_external_bot(self, client, auth_headers, season):
        r = await client.post(
            ...
            json={
                "name": "Zurich Crusher",
                "external_team_name": "Team Zurich",
                "season_id": season.id,
            },
        )
        assert r.status_code == 201
    ...
    async def test_update_and_delete(self, client, auth_headers, team, db):
        c = await client.post(
            "/api/bots", headers=auth_headers, json={"name": "Tmp", "team_id": team.id}
        )
        bid = c.json()["id"]
        await db.commit()
...
class TestBotFilters:
    ...
    async def test_filter_internal_vs_external(self, client, auth_headers, season, team, db):
        await client.post(
            "/api/bots",
            headers=auth_headers,
            json={"name": "Mine", "team_id": team.id, "season_id": season.id},
        )
        await client.post(
            "/api/bots",
            headers=auth_headers,
            json={"name": "Theirs", "external_team_name": "Ext", "season_id": season.id},
        )
        await db.commit()
...
class TestBotImage:
    ...
    async def test_upload_valid_png(self, client, auth_headers, team, db):
        c = await client.post(
            "/api/bots", headers=auth_headers, json={"name": "ImgBot", "team_id": team.id}
        )
        bid = c.json()["id"]
        await db.commit()
    ...
    async def test_upload_rejects_non_image(self, client, auth_headers, team, db):
        c = await client.post(
            "/api/bots", headers=auth_headers, json={"name": "BadImg", "team_id": team.id}
        )
        bid = c.json()["id"]
        await db.commit()
    ...
    async def test_image_404_when_none_uploaded(self, client, auth_headers, team, db):
        c = await client.post(
            "/api/bots", headers=auth_headers, json={"name": "NoImg", "team_id": team.id}
        )
        bid = c.json()["id"]
        await db.commit()
