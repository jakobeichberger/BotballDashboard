...
class TestSeasonEvents:
    ...
    async def test_delete_missing_event_404(self, client, auth_headers, season):
        d = await client.delete(
            f"/api/seasons/{season.id}/events/does-not-exist", headers=auth_headers
        )
...
class TestCompetitionLevelCrud:
    ...
    async def test_create_patch_delete(self, client, auth_headers, db):
        ...
        p = await client.patch(
            f"/api/seasons/competition-levels/{lid}",
            headers=auth_headers,
        ...
        all_lvls = await client.get(
            "/api/seasons/competition-levels/all?include_inactive=true", headers=auth_headers
        )
        assert any(lvl["code"] == "SR" for lvl in all_lvls.json())
        active = await client.get("/api/seasons/competition-levels/all", headers=auth_headers)
        assert not any(lvl["code"] == "SR" for lvl in active.json())
    ...
    async def test_permissions_and_create_role(self, client, auth_headers, db):
        ...
        r = await client.post(
            ...
            json={
                "name": "head-juror",
                "description": "chief",
                "permission_names": ["scoring:read", "scoring:write"],
            },
        )
...
class TestPracticeMatches:
    ...
    async def test_practice_excluded_from_ranking(self, client, auth_headers, season, team, db):
        ...
        ranking = await client.get(
            f"/api/scoring/seasons/{season.id}/ranking", headers=auth_headers
        )
        rows = ranking.json()
        ...
        practice = await client.get(
            f"/api/scoring/seasons/{season.id}/matches?is_practice=true", headers=auth_headers
        )
...
class TestPaperFinalize:
    ...
    async def test_finalize_aggregates_and_ranks(self, client, auth_headers, season, team, db):
        ...
        reviewer = User(
            ...
            is_active=True,
            is_superuser=True,
        )  # superuser to bypass papers:review gate
        db.add(reviewer)
        await db.commit()
