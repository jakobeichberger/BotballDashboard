...
async def schema(db, season):
    db.add(
        ScoringSchema(season_id=season.id, fields=[{"key": "pts", "multiplier": 1}], is_active=True)
    )
    await db.commit()
    ...
    async def test_a_teams_rank_ignores_teams_in_other_levels(
        ...
        await client.post(
            ...
            json={
                "team_id": c.id,
                "round_number": 1,
                "competition_level_id": junior.id,
                "raw_scores": {"pts": 99},
            },
        )
        await db.commit()
        ...
        other_level = "a higher-scoring team in another level must not push this one down"
        assert by_team[a.id] == 1, other_level
...
class TestQuotaIsUniquePerTeamAndEvent:
    @pytest.mark.asyncio
    async def test_repeated_reads_reuse_one_row(self, client, auth_headers, db, season, team):
        first = await client.get(
            f"/api/printing/quotas?team_id={team.id}&season_id={season.id}", headers=auth_headers
        )
        await db.commit()
    ...
    async def test_model_declares_the_unique_constraint(self):
        """Quotas are scoped to an event, and the constraint is what makes a lost
        insert race fail cleanly (see _get_or_create_quota's savepoint)."""
        ...
        assert ("event_id", "team_id") in uniques
