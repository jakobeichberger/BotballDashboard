...
class TestMentorCannotTouchOtherTeams:
    ...
    async def test_bulk_create_is_scoped_like_single_create(self, client, db, season, team, rival):
        ...
        resp = await client.post(
            f"/api/scoring/seasons/{season.id}/matches/bulk", headers=headers, json=payload
        )
    ...
    @pytest.mark.asyncio
    async def test_cannot_patch_another_teams_match(
    ...
    async def test_cannot_patch_another_teams_paper(
        ...
        _, headers = await _mentor(db, team)
        resp = await client.patch(
            f"/api/papers/{paper_id}", headers=headers, json={"title": "Hijacked"}
        )
        assert resp.status_code == 403
...
class TestDraftAnnouncementsArePrivate:
    ...
    async def test_plain_user_cannot_list_unpublished(self, client, db):
        ...
        resp = await client.get(
            "/api/dashboard/announcements?include_unpublished=true", headers=headers
        )
        assert resp.status_code == 403
    ...
    async def test_admin_can_list_unpublished(self, client, auth_headers):
        resp = await client.get(
            "/api/dashboard/announcements?include_unpublished=true", headers=auth_headers
        )
...
class TestDisqualificationUpdatesRanking:
    @pytest.mark.asyncio
    async def test_disqualified_match_leaves_the_ranking(
        self, client, db, season, team, auth_headers
    ):
        db.add(
            ScoringSchema(
                season_id=season.id, fields=[{"key": "pts", "multiplier": 1}], is_active=True
            )
        )
        await db.commit()
        ...
        await client.patch(
            f"/api/scoring/matches/{big.json()['id']}",
            headers=auth_headers,
...
class TestReviewScoreBounds:
    ...
    async def test_criteria_above_ten_are_rejected(self, client, db, season, team, auth_headers):
        ...
        too_high = await client.put(
            f"/api/papers/{pid}/reviews?submit=true", headers=headers, json={"score_content": 100}
        )
        assert too_high.status_code == 422

        negative = await client.put(
            f"/api/papers/{pid}/reviews?submit=true", headers=headers, json={"score_content": -5}
        )
