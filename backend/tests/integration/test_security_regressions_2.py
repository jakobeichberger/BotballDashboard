...
class TestScoreCannotBeForged:
    ...
    async def test_total_score_cannot_be_injected(self, client, db, season, team, schema):
        ...
        patched = await client.patch(
            f"/api/scoring/matches/{created.json()['id']}",
            headers=headers,
    ...
    @pytest.mark.asyncio
    async def test_unknown_fields_score_nothing(
        self, client, db, season, team, schema, auth_headers
    ):
        created = await client.post(
            f"/api/scoring/seasons/{season.id}/matches",
            headers=auth_headers,
            json={"team_id": team.id, "round_number": 1, "raw_scores": {"totally_made_up": 50000}},
        )
        # An invented field must never score. validate_raw_scores rejects it
        # outright rather than silently dropping it, so the juror sees the error.
        assert created.status_code == 422
        assert "not part of the active scoring schema" in created.json()["message"]
        listed = await client.get(f"/api/scoring/seasons/{season.id}/matches", headers=auth_headers)
        assert listed.json() == []

    @pytest.mark.asyncio
    async def test_values_above_max_are_rejected(
        self, client, db, season, team, schema, auth_headers
    ):
        ...
        created = await client.post(
            f"/api/scoring/seasons/{season.id}/matches",
            headers=auth_headers,
            json={"team_id": team.id, "round_number": 1, "raw_scores": {"cubes": 99999}},
        )
        assert created.status_code == 422
...
class TestReviewsAreNotLeakedByGetPaper:
    @pytest.mark.asyncio
    async def test_reviews_are_filtered_for_non_admins(
        self, client, db, season, team, auth_headers
    ):
...
class TestBotImageIsServedSafely:
    ...
    async def test_html_named_image_is_not_served_as_html(self, client, db, team, auth_headers):
        bot = await client.post(
            "/api/bots", headers=auth_headers, json={"name": "XSS", "team_id": team.id}
        )
        bid = bot.json()["id"]
        await db.commit()

        up = await client.post(
            f"/api/bots/{bid}/image",
            headers=auth_headers,
