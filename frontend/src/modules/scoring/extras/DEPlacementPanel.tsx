...
export default function DEPlacementPanel({ eventId }: { eventId: string }) {
  ...
  return (
    <section className="card overflow-hidden" aria-labelledby="de-placement-title">
      <h2 id="de-placement-title" className="border-b px-4 py-3 font-semibold dark:border-gray-800">{t("de.placement")}</h2>
      <table className="w-full text-sm">
        <thead className="bg-gray-50 dark:bg-gray-800"><tr><th className="px-4 py-2 text-left">{t("de.bracket")}</th><th className="px-4 py-2 text-left">{t("de.place")}</th><th className="px-4 py-2 text-left">{t("scouting.team")}</th><th className="px-4 py-2 text-left">{t("de.rank")}</th><th className="px-4 py-2 text-left">{t("events:tiebreaker")}</th></tr></thead>
