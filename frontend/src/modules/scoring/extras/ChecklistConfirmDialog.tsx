...
export default function ChecklistConfirmDialog({ open, items, pending, error, onCancel, onConfirm }: Props) {
  ...
  return (
    <Modal open={open} title="Schiedsrichter-Checkliste" onClose={onCancel}>
      <ul className="space-y-2">
        {items.map((item) => (
          <li key={item.key}>
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" checked={!!checked[item.key]} onChange={(e) => setChecked({ ...checked, [item.key]: e.target.checked })} />
              {item.label}{item.required && <span className="text-red-600" aria-label="Pflicht">*</span>}
            </label>
          </li>
        ))}
      </ul>
      ...
      <div className="mt-4 flex justify-end gap-2">
        ...
        <button type="button" className="btn-primary" disabled={missing.length > 0 || pending} onClick={() => onConfirm(checked)}>Score bestätigen</button>
      </div>
    </Modal>
  );
}
