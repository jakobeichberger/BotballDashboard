...
const STATUS_LABEL: Record<QueuedScore["status"], string> = {
  ...
  return (
    <section aria-labelledby="pending-scores-title" className="card border-amber-300 p-4 dark:border-amber-700">
      <h2 id="pending-scores-title" className="mb-3 flex items-center gap-2 font-semibold">
        <CloudOff className="h-4 w-4" aria-hidden="true" /> Noch nicht synchronisiert ({entries.length})
      </h2>
            ...
            <li key={entry.id} className="rounded-lg border p-3 text-sm dark:border-gray-700">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div className="min-w-0">
                  <p className="truncate font-medium">{entry.label || "Wertung"}</p>
              ...
              {failed && (
                <div className="mt-2 flex flex-wrap gap-2">
                  <button type="button" className="btn-secondary text-xs" onClick={() => void retry(entry.id)}>
                    <RotateCcw className="h-3 w-3" aria-hidden="true" /> Erneut versuchen
                  </button>
                  ...
                  <button type="button" className="btn-secondary text-xs text-red-600" onClick={() => { if (window.confirm("Diese Wertung verwerfen?")) void discard(entry.id); }}>
                    <Trash2 className="h-3 w-3" aria-hidden="true" /> Verwerfen
                  </button>
                </div>
              )}
            </li>
