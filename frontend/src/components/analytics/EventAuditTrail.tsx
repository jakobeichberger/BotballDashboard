  ...
  const changeList = (changes: FieldChange[]) =>
    changes.length ? (
      <ul className="space-y-0.5">
        {changes.map((change) => <li key={change.key}><span className="text-gray-500">{fieldLabel(change.key)}:</span> {value(change.before)} → <strong>{value(change.after)}</strong></li>)}
      </ul>
    ) : <span className="text-gray-400">{t("audit.noChange")}</span>;

  const rows = tab === "scores" ? scores : results;
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap items-center gap-3">
        <div role="tablist" aria-label={t("audit.title")} className="inline-flex overflow-hidden rounded-lg border border-gray-200 dark:border-gray-700">
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[640px] text-sm">
          ...
          <tbody>
            {tab === "scores" && scores.data?.map((rev) => {
              ...
              return (
                <tr key={rev.id} className="border-t border-gray-100 align-top dark:border-gray-800">
                  ...
                  <td className="py-1.5 pr-3">
                    ...
                    <span className="ml-1 text-xs text-gray-500">{t("audit.revision", { revision: rev.revision })}</span>
                    {rev.reason && <p className="mt-0.5 text-xs text-gray-600 dark:text-gray-400">{rev.reason}</p>}
                  </td>
                  <td className="py-1.5 pr-3">
        ...
        {!rows.isLoading && !rows.data?.length && <p className="py-4 text-center text-sm text-gray-400">{t("audit.empty")}</p>}
      </div>
    </div>
  );
}
