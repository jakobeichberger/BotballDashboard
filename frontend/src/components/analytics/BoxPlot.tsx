...
export function BoxPlotRow({
  ...
  return (
    <div className="grid grid-cols-[8rem_1fr_4rem] items-center gap-3 py-1.5" role="listitem">
      <span className="truncate text-sm text-fg" title={label}>{label}</span>
...
export function BoxPlotList({
  ...
  if (!withData.length) return <p className="text-sm text-leise">{t("noData")}</p>;
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-[8rem_1fr_4rem] gap-3 text-xs text-leise tabular-nums">
        <span />
