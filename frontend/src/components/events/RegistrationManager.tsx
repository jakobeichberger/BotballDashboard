  ...
  return (
    <section className="card p-5">
      ...
      <form className="grid gap-2 sm:grid-cols-[1fr_8rem_auto]" onSubmit={(e) => { e.preventDefault(); addTeam.mutate(); }}>
        ...
        <select aria-label={t("setup.category")} className="input" value={teamCategory} onChange={(e) => setTeamCategory(e.target.value)}>{categories.categories.map((category) => <option key={category.key} value={category.key}>{categories.label(category.key)}</option>)}</select>
        ...
        {list.map((item) => (
          <li key={item.id} className="flex items-center justify-between gap-2 py-1.5">
            <label className="flex min-w-0 items-center gap-2">
              ...
              />
              <span className="truncate">{item.team_name}{item.team_number ? ` (${item.team_number})` : ""} · {categories.label(item.category)}</span>
            </label>
