...
function ProtectedCrop({ url, label }: { url: string; label: string }) {
  ...
  return src ? <img src={src} alt={t("scans.cropAlt", { label })} className="h-20 w-full rounded border bg-white object-contain" /> : <div className="h-20 animate-pulse rounded bg-gray-100 dark:bg-gray-800" />;
}
...
function ReviewCard({ scan, teamLabel }: { scan: Scan; teamLabel: string }) {
  ...
  return <article className="card p-4"><div className="mb-4 flex items-center justify-between"><div><h2 className="font-semibold">{scan.file_name}</h2><p className="text-xs text-gray-500">{t("scans.team", { team: teamLabel })}</p></div><span className="badge-blue">{t("scans.reviewRequired")}</span></div><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{scan.extracted_values?.map((item) => <div key={item.key} className={`rounded-lg border p-3 ${item.requiresReview ? "border-amber-400 bg-amber-50 dark:bg-amber-950/20" : "dark:border-gray-700"}`}><label htmlFor={fieldId(item.key)} className="mb-2 block text-sm font-medium">{item.key}</label><ProtectedCrop url={item.cropUrl} label={item.key} /><input id={fieldId(item.key)} aria-describedby={`${fieldId(item.key)}-hint`} className="input mt-2 w-full" type="number" value={Number(values[item.key] ?? 0)} onChange={(e) => setValues((current) => ({ ...current, [item.key]: Number(e.target.value) }))} /><span id={`${fieldId(item.key)}-hint`} className="mt-1 block text-xs text-gray-500">{t("scans.confidence", { percent: Math.round(item.confidence * 100) })} {item.reasons.map((reason) => t(`scans.reason.${reason}`, { defaultValue: reason })).join(", ")}</span></div>)}</div>{accept.isError && <p role="alert" className="mt-3 text-sm text-red-600">{apiErrorMessage(accept.error, t("common:actionFailed"))}</p>}<button type="button" className="btn-primary mt-4 flex min-h-11 items-center gap-2" onClick={() => accept.mutate()} disabled={accept.isPending}><CheckCircle2 className="h-4 w-4" aria-hidden="true" />{t("scans.accept")}</button></article>;
}
...
export default function ScanReviewPage() {
  ...
  return <div className="mx-auto max-w-7xl space-y-6 p-4 md:p-6"><h1 className="flex items-center gap-2 text-2xl font-bold"><ScanLine />{t("scans.title")}</h1><form className="card grid gap-3 p-4 md:grid-cols-[1fr_1fr_1fr_auto]" onSubmit={(e) => { e.preventDefault(); upload.mutate(); }}><select required aria-label={t("scans.template")} className="input" value={templateId} onChange={(e) => setTemplateId(e.target.value)}><option value="">{t("scans.chooseTemplate")}</option>{templates.data?.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select><select required aria-label={t("scouting.team")} className="input" value={teamId} onChange={(e) => setTeamId(e.target.value)}><option value="">{t("scouting.chooseTeam")}</option>{sortedRegistrations.map((item) => <option key={item.id} value={item.team_id}>{registrationLabel(item)}</option>)}</select><div className="flex flex-col gap-2"><input className="input" aria-label={t("scans.chooseFile")} type="file" accept="image/jpeg,image/png,image/webp,application/pdf" onChange={(e) => setFile(e.target.files?.[0] ?? null)} />{/* capture opens the rear camera directly on phones; desktops ignore it and show the file picker. */}<label className="btn-secondary flex cursor-pointer items-center justify-center gap-2 md:hidden"><Camera className="h-4 w-4" aria-hidden="true" />{t("scans.takePhoto")}<input className="sr-only" type="file" accept="image/*" capture="environment" onChange={(e) => setFile(e.target.files?.[0] ?? null)} /></label>{file && <span className="truncate text-xs text-gray-500">{file.name}</span>}</div><button className="btn-primary flex min-h-11 items-center justify-center gap-2" disabled={upload.isPending || !file}><Upload className="h-4 w-4" aria-hidden="true" />{t("common:upload")}</button>{upload.isError && <p role="alert" className="text-sm text-red-600 md:col-span-4">{apiErrorMessage(upload.error, t("common:actionFailed"))}</p>}</form><Freshness query={scans} live={false} /><div className="space-y-5">{retryError && <p role="alert" className="text-sm text-red-600">{retryError}</p>}{scans.data?.filter((scan) => scan.status === "review").map((scan) => <ReviewCard key={scan.id} scan={scan} teamLabel={teamLabel(scan.team_id)} />)}{scans.data?.filter((scan) => ["queued", "processing", "failed"].includes(scan.status)).map((scan) => <div key={scan.id} className="card flex items-center justify-between p-4"><div><p className="font-medium">{scan.file_name} · {teamLabel(scan.team_id)}</p><p className="text-sm text-gray-500">{SCAN_STATUSES.includes(scan.status) ? t(`scans.status.${scan.status}`) : scan.status}{scan.error ? ` · ${scan.error}` : ""}</p></div>{scan.status === "processing" || !canWrite ? <RefreshCw className={scan.status === "processing" ? "animate-spin" : ""} aria-hidden="true" /> : <button type="button" className="btn-secondary flex items-center gap-2" disabled={retry.isPending} aria-label={t("scans.retryFor", { file: scan.file_name })} onClick={() => retry.mutate(scan.id)}><RotateCcw className="h-4 w-4" aria-hidden="true" />{t("scans.retry")}</button>}</div>)}{scans.isLoading && <p role="status" className="text-sm text-gray-500">{t("common:loadingEllipsis")}</p>}{scans.isSuccess && !scans.data?.some((scan) => scan.status !== "accepted") && <div className="card p-8 text-center text-gray-500">{t("scans.none")}</div>}</div></div>;
}
