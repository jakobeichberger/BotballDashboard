...
export default function ScanReviewPage() {
  ...
  return <div className="mx-auto max-w-7xl space-y-6 p-4 md:p-6"><h1 className="flex items-center gap-2 text-2xl font-bold"><ScanLine />Lokale OCR-Prüfung</h1><form className="card grid gap-3 p-4 md:grid-cols-[1fr_1fr_1fr_auto]" onSubmit={(e) => { e.preventDefault(); upload.mutate(); }}><select required className="input" value={templateId} onChange={(e) => setTemplateId(e.target.value)}><option value="">Vorlage wählen</option>{templates.data?.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select><select required aria-label="Team" className="input" value={teamId} onChange={(e) => setTeamId(e.target.value)}><option value="">Team wählen</option>{sortedRegistrations.map((item) => <option key={item.id} value={item.team_id}>{registrationLabel(item)}</option>)}</select><div className="flex flex-col gap-2"><input className="input" aria-label="Datei wählen" type="file" accept="image/jpeg,image/png,image/webp,application/pdf" onChange={(e) => setFile(e.target.files?.[0] ?? null)} />{/* capture opens the rear camera directly on phones; desktops ignore it and show the file picker. */}<label className="btn-secondary flex cursor-pointer items-center justify-center gap-2 md:hidden"><Camera className="h-4 w-4" aria-hidden="true" />Foto aufnehmen<input className="sr-only" type="file" accept="image/*" capture="environment" onChange={(e) => setFile(e.target.files?.[0] ?? null)} /></label>{file && <span className="truncate text-xs text-gray-500">{file.name}</span>}</div><button className="btn-primary flex items-center justify-center gap-2" disabled={upload.isPending || !file}><Upload className="h-4 w-4" />Hochladen</button></form><div className="space-y-5">{scans.data?.filter((scan) => scan.status === "review").map((scan) => <ReviewCard key={scan.id} scan={scan} teamLabel={teamLabel(scan.team_id)} />)}{scans.data?.filter((scan) => ["queued", "processing", "failed"].includes(scan.status)).map((scan) => <div key={scan.id} className="card flex items-center justify-between p-4"><div><p className="font-medium">{scan.file_name} · {teamLabel(scan.team_id)}</p><p className="text-sm text-gray-500">{scan.status}{scan.error ? ` · ${scan.error}` : ""}</p></div><RefreshCw className={scan.status === "processing" ? "animate-spin" : ""} /></div>)}{!scans.isLoading && !scans.data?.some((scan) => scan.status !== "accepted") && <div className="card p-8 text-center text-gray-500">Keine offenen Scans.</div>}</div></div>;
}
