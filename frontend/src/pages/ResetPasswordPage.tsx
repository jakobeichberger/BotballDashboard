...
export default function ResetPasswordPage() {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setError(apiErrorMessage(err, "Das Passwort konnte nicht gesetzt werden."));
    } finally {
      setBusy(false);
    }
  };
        ...
        <div className="card p-6 space-y-4">
          {!token ? (
            ...
            <p role="status" className="text-sm text-gray-700 dark:text-gray-300">
              Dein Passwort wurde geändert. Alle bisherigen Sitzungen wurden beendet.
            </p>
          ) : (
            <form onSubmit={submit} className="space-y-4">
              ...
              <div>
                <label className="label" htmlFor="confirm-password">Passwort wiederholen</label>
                <input
                  id="confirm-password"
                  type="password"
                  className="input"
          ...
          <p className="text-center text-sm">
            <Link to={done ? "/login" : "/forgot-password"} className="text-primary-600 hover:underline dark:text-primary-400">
              {done ? "Zur Anmeldung" : "Neuen Link anfordern"}
            </Link>
          </p>
        </div>
      </div>
