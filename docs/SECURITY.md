...
- Public event endpoints only serve events in `published`, `live` or `completed`, and each view requires its `public_*` flag. The public WebSocket requires at least one flag. The authenticated event WebSocket (`/api/v1/events/{event_id}/ws`) takes the access token in its first message (never in the URL), checks it like any request plus `events:read` and the draft rule, re-checks every 60 s and at the token's expiry, and closes with 4401/4403/4404.
...
- **bcrypt:** runs in a worker thread (`hash_password_async` / `verify_password_async`), never on the event loop. A login for an unknown or inactive address is checked against a dummy hash, so the response time does not reveal which addresses have an account. Verification cuts the entered password to 72 bytes, exactly as bcrypt 4 did when hashing, so accounts with a longer password from before bcrypt 5 still log in, and an over-long password at login is an ordinary failed attempt rather than an error.
- **Password policy:** at least 10 characters, at most 72 bytes in UTF-8 (bcrypt's input limit; bcrypt 5 refuses longer passwords instead of truncating them, umlauts count 2 bytes, emoji 4), not a single repeated character, not the e-mail address, and not on the bundled list of ~2,300 common/leaked passwords (`backend/modules/auth/common_passwords.txt`, from SecLists, case-insensitive). Applies to account creation, password change, reset and admin-set.
...
7. **Traefik buffering — Info.** The body-limit middleware buffers requests (on disk above 2 MB) and responses before forwarding them; large downloads reach the client only after Traefik has received them completely. The live WebSockets have their own router without it.
